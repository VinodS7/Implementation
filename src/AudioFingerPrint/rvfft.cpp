...
void dif_butterfly(float *data,long size){
    ...
    long angle,astep,dl;
    float xr,yr,xi,yi,wr,wi,dr,di,ang;
    float *l1, *l2, *end, *ol2;
     ...
     for(dl=size;dl>1;dl>>=1,astep+=astep){
                      ...
                      for(;l2<end;l1=l2,l2=l2+dl){
                            ...
                            for(angle=0;l1<ol2;l1+=2,l2+=2){
                               ang=static_cast<float>(2*M_PI*angle/size);
                               wr=cosf(ang);
                               wi=-sinf(ang);
                               xr=*l1+*l2;
                               xi=*(l1+1)+*(l2+1);
...
void realize(float *data, long size){
    ...
    float xr,yr,xi,yi,wr,wi,dr,di,ang,astep;
    float *l1, *l2;
    ...
    *(l1+1)=xr-xi; 
    l1+=2;
    astep=static_cast<float>(M_PI/size);
...
// cosfinfe is calculated only once;
...
void realfft_bruun(float *data, long size){
    ...
    *l1=-(*l1);
    ...
    p4=static_cast<float>(M_PI/(2*size));
    j=1;
    kk=1;
    dl_o=size/2;
    dl2_o=size/4;
...
////
//// Source: see the routines it calls ...
//void realfft_bruun_unshuffled(float *data, long size){
//
//    float *data2;
//    long i,j,k;
//
...
//    data2=(float *)malloc(size*sizeof(float));
...
// much cosfinfe calls - with a cosfinfe lookup table
// probably this would be slightly faster than bruun_fft

/*void realfft_bruun2(float *data, long size){
    float *end, *l0, *l1, *l2, *l3;
    long dl, dl2, i, j;
    float d0,d1,d2,d3,c,c2,p4;
    end=data+size;
    p4=M_PI/(size);
    ...
    for(;dl>1;dl>>=1,dl2>>=1){
        ...
        while(l3<end){
            ...
            c=2*cosf(p4*bruun_reverse(j,size));
            for(i=0;i<dl2;i++){
                d0=*l0;
                d1=*l1;
                d2=*l2;
...
/*void realfft_bruun_unshuffled2(float *data, long size){
    float *data2;
    long i,j,k;
...
//// Source: see the routines it calls ...
//
//void realfft_radix2_unshuffled(float *data,long n){
//    
//    float *data2;
//    long i,j;
//
